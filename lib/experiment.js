...
module.exports = function(experiments) {
  ...
  Experiment.prototype.stateFor = function stateFor(context) {
    if(this.variants.length === 1) {
      return this.variants[0].contains(context.bucket, context.userId);
    }

    var feature = {};
    for (var i = 0, len = this.variants.length; i < len; ++i) {
      var variant = this.variants[i];
      feature[variant.name] = variant.contains(context.bucket, context.userId);
    }
  ...
  Experiment.forContext = function forContext(context, exps) {
    ...
    if(result.stamp !== context.stamp) {
      result = {};
    }

    result.stamp = context.stamp;
    result.bucket = context.bucket;
