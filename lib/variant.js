...
Variant.prototype.setPercentage = function setPercentage(lowerBound, upperBound) {
    ...
    this.upperBound = Math.min(parseInt(upperBound, 10), 100);
    if (isNaN(this.upperBound)) this.upperBound = 100;
}

/**
 * Returns `true` if isOn is `true` OR if this variant contains the user with the given `userId` and isOn is null, false otherwise.
 */
Variant.prototype.contains = function contains(bucket, userId) {
    if(this.isOn !== null) {
        return this.isOn;
    }
    ...
    for (var i = 0, len = this.groups.length; i < len; ++i) {
        group = this.groups[i];
        if (group.contains(bucket, userId)) {
            return true;
        }
    }
